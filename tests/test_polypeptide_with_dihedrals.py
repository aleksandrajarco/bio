...
class TestPolypeptideWithDihedrals(unittest.TestCase):
    def setUp(self):
        ...
        self.second_res_with_angles = next(
            islice(phi_psi_list.items(), 1, 2)
        )  # first residue does not contain the phi/psi angles
    ...
    def test_polypeptide(self):
        self.assertGreater(
            len(self.polypeptide_with_dihedrals), 0
        )  # add assertion here
    ...
    def test_assign_secondary_structure(self):
        ...
        self.assertIn(
            ss_map[self.second_res], ("U", "alpha", "beta", "b_par", "b_anti")
        )
